...
$$kurt(y)=\mathbb E[y^4] - 3 \mathbb E[y^2]^3 \tag{7}$$

根据假设，$y$ 方差为 1，那么 (7) 式右侧简化为 $\mathbb E[y^4]-3$。

如果 $y$ 是高斯变量，那么 $kurt(y)=3 \mathbb E[y^2]^2-3=0$ 。其他随机变量，峰度可正可负。

峰度具有线性性质：

1. $kurt(x_1+x_2)=kurt(x_1)+kurt(x_2)$
2. $kurt(ax_1)=a^4 kurt(x_1)$

**峰度的例子**

考虑二维模型 $\mathbf x = A \mathbf s$，假设独立成分的峰度 $kurt(s_1), \ kurt(s_2)$ 均不为 0，之前假设了随机变量方差为 1，那么根据 (6) 式 $y=z_1s_1+z_2s_2$ 和峰度的线性性质可知

$$kurt(y)=kurt(z_1s_1)+kurt(z_2s_2)=z_1^4 kurt(s_1) + z_2^4 kurt(s_2)$$

另一方面，我们有办法使 $y$ 的方差为 1，前文假设了独立成分的均值为 $0$，那么有 

$$\begin{aligned}\mathbb E[y^2]&=\mathbb E[z_1^2s_1^2+z_2^2s_2^2+2z_1z_2s_1s_2]
\\&=z_1^2+z_2^2+2z_1z_2 \mathbb E[s_1s_2]
\\&= z_1^2 + z_2^2=1
\end{aligned}$$

问题转为：

$$\max_{\mathbf z} \ z_1^4 kurt(s_1) + z_2^4 kurt(s_2) \quad s.t. \ ||\mathbf z||=1$$

不妨考虑峰度均为正的情况，$kurt(s_1), \ kurt(s_2) > 0$，那么当 

1. $kurt(s_1)\ge \ kurt(s_2)$ 时，$z_1=\pm 1, z_2=0$ 为最优解
2. $kurt(s_1) < \ kurt(s_2)$ 时，$z_1=0, z_2=\pm 1$ 为最优解
