...
仍以语音识别为例，如果语音开头有沉默，那么 RNN 输出序列 $A$ 以 $\epsilon$ 开头，否则以 $y_1$ 开头。具体而言，将语音等分为 $T$ 个语音片，即 $A=[a_1,a_2,\ldots,a_T]$，第 1 个语音片为沉默（无声音），那么对应 $\epsilon$，否则对应 $y_1$，第 2 个语音片则需要根据上一个语音片的情况进行讨论，如下表所示，
...
\hline
\\\\
a _ 1=y _ 1 & \begin{aligned}a _ 2&=\epsilon \\\\ a _ 2&=y _ 1 \\\\ a _ 2&=y _ 2 (y _ 2\neq y _ 1)\end{aligned} & \begin{aligned} &(y _ 1, \epsilon) \\\\ &(y _ 1,y _ 1) \\\\ &(y _ 1,y _ 2)\end{aligned}
\\\\
\hline
\\\\
a _ 1=\epsilon & \begin{aligned}a _ 2&=\epsilon \\\\ a _ 2&=y _ 1\end{aligned} & \begin{aligned} &(\epsilon, \epsilon) \\\\ &(\epsilon,y _ 1)\end{aligned}
\end{array}$$

注意，$a _ 1=y _ 1, \ a _ 2=y _ 2$ 时，必须保证 $y _ 1\neq y _ 2$ 这个条件成立，否则 $B(a _ 1,a _ 2)=B(y _ 1,y _ 1)=(y _ 1)$，即，经过合并后，就少了一个 $y _ 1$。
...
$$\alpha_{s,t}=P\lbrace B(A_{1:t}) = C(Z_{1:s})\rbrace$$

其中 $C$ 表示适当地去掉下方 case 3 中的 `eps`，使得 $B(A_{1:t})$ 与 $C(Z_{1:s})$ 相等。

对于 $Z$ 中连续的三个元素 $z_{s-2}, z_{s-1},z_s$，这三个元素所有可能的情况为

```sh
case_idx    z(s-2)  z(s-1)  z(s)
1           a       eps     a
2           eps     a       eps
3           a       eps     b
```

根据是否可以跳过 $z _ {s-1}$，可以将这三种情况分为两大类：

1. **无法跳过 $z_{s-1}$**，即，必须有一个 RNN 输出 $a_j$ 对齐到 $z_{s-1}$。

    case 1 和 2 属于此类。case 1 中的 `eps` 如果不预测，那么两个连续的 `a` 经过去重变成一个 `a`，而原来 gt $Y$ 中这里是两个相同的 token `a`；case 2 中的 `a` 显然不能跳过。
    ...
    $\alpha_{s,t}$ 可由 $\alpha_{s,t-1}$ 转化而来，即 $a_t=a_{t-1}$，也可由 $\alpha_{s-1,t-1}$ 转化而来，即 $a_t \neq a_{t-1}$，
...
2. **允许跳过 $z_{s-1}$**，允许跳过，即，可以跳过，也可以不跳过。
    ...
    如图 2 所示，对应上面的 case 3，$z_{s-1}=\epsilon$，且 $z_{s-2}\neq z_s$

    ![](/images/dl/CTC2.png)

    <center>图 2 （来源于 ref 1）</center>

    此时，$\alpha_{s,t}$ 可由 $\alpha_{s-2,t-1}$ 或 $\alpha_{s-1,t-1}$ 或者 $\alpha_{s,t-1}$ 转化而来。
...
以图 3 为例进行说明，根据 $X$ 得到最终的序列预测得分为

```python
L= 6    # 序列长度
N = 4   # 每个 timestep 预测 3 个类别的得分，a, b, c, eps
labels = ['eps', 'a', 'b', 'c']
labels = {x: i for i, x in enumerate(labels)}
X = torch.randn((L, N))
X = torch.softmax(p, dim=-1)    # 转为归一化预测概率
# 计算 ctc loss： -log p(Y|X)
Y = ['a', 'b']
Z = ['eps' if i % 2 == 0 else Y[i // 2] for i in range(5)]  # eps, a, eps, b, eps
Z = [labels[z] for z in Z]

alpha = torch.zeros((len(Z), L))
for t in range(L):
    for s, z in enumerate(Z):
        if t == 0:
            alpha[s, t] = X[t, z]
        else:
            if s == 0:
                alpha[s, t] = alpha[s, t-1] * X[t, z]
            elif s == 1:
                alpha[s, t] = (alpha[s-1, t-1] + alpha[s, t-1]) * X[t, z]
            else:   # s >= 2
                if Z[s-2] != z: # 情况 2
                    alpha[s, t] = (alpha[s-2, t-1] + alpha[s-1, t-1] + alpha[s, t-1]) * X[t, z]
                else:           # 情况 1
                    alpha[s, t] = (alpha[s-1, t-1] + alpha[s, t-1]) * X[t, z]

prob = alpha[-1, -1] + alpha[-2, -1]    # 如图 3， 右下角的两个 nodes 是有效的，P(Y|X) 为两个 nodes 处 alpha 之和
```


**pytorch 中 CTCLoss 类说明**

```python
CLASStorch.nn.CTCLoss(blank=0, reduction='mean', zero_infinity=False)
```

1. blank 是 `eps` 的 label id，默认为 0
2. zero_infinity 用于输入序列长度太短无法对齐 targets

forward 方法参数：

1. `Log_probs`，各分类的预测得分（非归一化，概率对数），shape 为 $(T, N, C)$，表示序列长度，batch size，以及分类数量（包括 eps）
2. `Targets`， gt 序列。$(N, S)$，表示 batch size，以及 batch 中最大序列长度
3. `Input_lengths`，各个输入序列的长度，$(N,)$，通常每个输入序列长度相等，均为 T
4. `Target_lengths`，各个 gt 序列的长度，$(N, )$，各个 gt 序列长度不一定相等
